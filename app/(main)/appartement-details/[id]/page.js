  ...
  useEffect(()=>{
    const details=async ()=> {
      try{
        ...
        const request= await http.get(`/accommodations?accommodation_id=${params.id}&limit=1`);
            ...
            <div className="flex justify-between items-start">
              <div className="h-[120px]">
                ...
                <div className="flex items-center text-gray-600   mt-2 font-montserrat">
                  ...
                  <span className="text-sm">{pageData.address}, {pageData.country}</span>
                </div>
                <div className="text-xl  mt-2  font-bold text-gray-900 font-montserrat-bold">
                  {pageData ? pageData?.price_per_night : property.price} {property.currency}/ {property.period}
                </div>
              </div>
      ...
      : "stroke-yellow-500 text-gray-400"}
            ...
            <div className="grid grid-cols-3 mt-10  gap-1 w-[580px]">
              <div className="border-2 border-primary pl-4 pr-4 pt-10 w-[176px] h-[160px] rounded-lg  p-5 text-center hover:shadow-md transition-shadow">
                ...
                <div className="text-md font-semibold">{pageData.number_of_rooms} chambres</div>
              ...
              <div className="border-2 w-[176px] h-[160px] border-primary rounded-lg pl-4 pr-4 pt-10 text-center hover:shadow-md transition-shadow">
                ...
                <div className="text-md font-semibold">{pageData.number_of_bathrooms} salles de bains</div>
              ...
              <div className="border-2 w-[176px] h-[160px] border-primary pl-4 pr-4 pt-10 rounded-lg p-6 text-center hover:shadow-md transition-shadow">
                ...
                <div className="text-md font-semibold">{pageData.number_of_parking} parking</div>
            ...
            <div className=''>
              ...
              <div className="grid grid-cols-2 gap-4">
                {(pageData?.amenities || property.amenities).map((amenity, index) => (
                  <div key={index} className="flex items-center space-x-3 p-2">
                    <SvgIcon name={typeof amenity === 'string' ? amenity : amenity.icon} size={24} className="filter " />
                    <span className="text-gray-700 font-medium">{typeof amenity === 'string' ? amenity : amenity.name}</span>
                  </div>
                ))}
              </div>
              <button className="mt-6 text-black border-2 px-7 py-4 rounded-lg border-primary font-semibold  transition-all">
                Afficher les {(pageData?.amenities || property.amenities).length} équipements
              </button>
            </div>
            ...
            <div className='mt-15 '>
              ...
              <div className="grid grid-cols-2 gap-4 w-[580px]">
                {(pageData?.securities || property.securityFeatures).map((feature, index) => (
                  <div key={index} className="flex items-center space-x-3 p-2">
